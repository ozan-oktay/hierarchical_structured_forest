...
# DEFINITIONS
    binary_display = os.path.join(malpem.mytools.__malpem_path__, "bin/display")
# END DEFINITIONS
...
# DEFINITIONS
    binary_volumes = os.path.join(malpem.mytools.__malpem_path__, "bin/cl_compute_volume")
# END DEFINITIONS
...
def create_report(input_file, input_mask, input_seg_malpem, input_seg_mori, output_report, output_dir):
# DEFINITIONS
    structure_names = os.path.join(malpem.mytools.__malpem_path__, "config/StructureInfo/NMM_Mori.csv")
    structure_lut = os.path.join(malpem.mytools.__malpem_path__, "config/Mori_lut.csv")
    ...
    cgm_list = range(41, 138, 1)
    dgm_list = range(1, 40, 1)
    wm_list = [12, 13, 16, 17]
    other_list = [14, 15, 33, 34, 18]
    for i in wm_list:
        dgm_list.remove(i)
    for i in vent_list:
        dgm_list.remove(i)
...
# CALCULATE STRUCTURAL VOLUMES
    ...
    mori_volume_file = os.path.join(report_dir, malpem.mytools.basename(output_report) + "_MALPEM_WM-Mori_raw.csv")
    malpem_report_file = os.path.join(report_dir, malpem.mytools.basename(output_report) + "_MALPEM_Report.csv")
    mori_report_file = os.path.join(report_dir, malpem.mytools.basename(output_report) + "_MALPEM_WM-Mori_Report.csv")

    calculate_volume(input_seg_malpem, malpem_volume_file)
    calculate_volume(input_seg_mori, mori_volume_file)
    ...
    f = open(mori_volume_file)
    for row in f:
        arr = row.split(',')
    for i in range(len(arr)):
        # TAKE ONLY VOLUMES THAT WERE NOT CALCULATED ABOVE
        if i >= len(s_volumes):
            s_volumes.append(arr[i].lstrip())
    f.close()
...
# TAKE RELEVANT SCREENSHOTS
    ...
    screenshots_mori = []
    screenshots_mori.append(os.path.join(report_dir, malpem.mytools.basename(output_report) + "_MALPEM_WM-Mori_xy.png"))
    screenshots_mori.append(os.path.join(report_dir, malpem.mytools.basename(output_report) + "_MALPEM_WM-Mori_xz.png"))
    screenshots_mori.append(os.path.join(report_dir, malpem.mytools.basename(output_report) + "_MALPEM_WM-Mori_yz.png"))
    ...
    take_screenshot(screenshots_mori[0], input_file + " -seg " + input_seg_mori + " -lut " + structure_lut + " -xy -res 2")
    take_screenshot(screenshots_mori[1], input_file + " -seg " + input_seg_mori + " -lut " + structure_lut + " -xz -res 2")
    take_screenshot(screenshots_mori[2], input_file + " -seg " + input_seg_mori + " -lut " + structure_lut + " -yz -res 2")
...
## START CREATING ACTUAL REPORT
    ...
    c.drawString(left_start, top_start-240-2*(20+img_width), "whole brain segmentation (MALPEM with Neuromorphometrics "
                                                             "+ Mori WM parcellation)")
    c.drawInlineImage(screenshots_mori[0], left_start, top_start-240-2*(20+img_width)-5-img_width, img_width, img_width)
    c.drawInlineImage(screenshots_mori[1], left_start+165, top_start-240-2*(20+img_width)-5-img_width, img_width, img_width)
    c.drawInlineImage(screenshots_mori[2], left_start+330, top_start-240-2*(20+img_width)-5-img_width, img_width, img_width)
    ...
    for i in range(len(s_volumes)):
        ...
        if i == 139:
            c.showPage()
            c.drawString(left_start, top_start-10, "Mori WM Volumes [ml] (these are subdivision of the NMM volumes)")
            c.line(left_start, top_start-15, right_stop, top_start-15)
            cur_line = 2
            cur_col = 0
    ...
    for i in range(len(s_id)):
        if i == 0:
            ...
            c.line(left_start, top_start-15, right_stop, top_start-10)
            cur_line += 2
        ...
        if i == 139:
            if cur_line > 0:
                c.showPage()
            cur_line = 0
            c.drawString(left_start, top_start-10, "Dictionary of Mori Structure Names "
                                                   "(remapped IDs from original Atlas)")
            c.line(left_start, top_start-15, right_stop, top_start-15)
            cur_line += 2
