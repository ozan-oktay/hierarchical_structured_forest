...
class pipeline(object):
    ...
    def reg(self, i, prev_method, cur_method, fr, mask): 
        if re.match('p\w*', cur_method):
            ...
            target = '{0}/fusion_pbcc_nreg_{1}_auto.nii.gz'.format(self.results_dir, fr)
            atlas = '{0}/Atlas{1:02d}/segmentation_{2}.nii.gz'.format(self.atlas_dir, i, fr)
        else:
            # Image-based non-rigid registration
            target = '{0}/cine_sax_{1}_crop_auto.nii.gz'.format(self.results_dir, fr)
            atlas = '{0}/Atlas{1:02d}/lvsa_{2}.nii.gz'.format(self.atlas_dir, i, fr)

        method = re.sub('_label', '', cur_method)
        cur_dof = '{0}/{1}_to_Atlas{2:02d}_{3}_auto.dof.gz'.format(self.dof_dir, cur_method, i, fr)

        # prreg does only have a ED dof file
        if re.match('p\w', prev_method):
            prev_dof = '{0}/{1}_to_Atlas{2:02d}_ED_auto.dof.gz'.format(self.dof_dir, prev_method, i)
        else:
            prev_dof = '{0}/{1}_to_Atlas{2:02d}_{3}_auto.dof.gz'.format(self.dof_dir, prev_method, i, fr)
        ...
        if mask:
            mask = '{0}/{1}_auto.nii.gz'.format(self.results_dir, mask)
    ...
    def transform(self, i, method, fr, crop):
        ...
        if crop:
            target_image = '{0}/cine_sax_{1}_{2}_auto.nii.gz'.format(self.results_dir, fr, crop)
        else:
        ...
        dof = '{0}/{1}_to_Atlas{2:02d}_{3}_auto.dof.gz'.format(self.dof_dir, method, i, fr)
        image_prop = '{0}/image_from_Atlas{1:02d}_{2}_{3}_auto.nii.gz'.format(self.warped_dir, i, method, fr)
        os.system('{0}/transformation {1} {2} -linear -dofin {3} -target {4}'.format(self.irtk_dir, atlas_image, image_prop, dof, target_image))
        label_prop = '{0}/label_from_Atlas{1:02d}_{2}_{3}_auto.nii.gz'.format(self.warped_dir, i, method, fr)
    ...
    def segmentation(self):
        ...
        fusion = '{0}/fusion_mv_prreg_ED_auto.nii.gz'.format(self.results_dir)
        ...
        fusion_crop = '{0}/fusion_mv_prreg_ED_crop_auto.nii.gz'.format(self.results_dir)
        os.system('auto_crop_image {0} {1} -reserve 20'.format(fusion, fusion_crop))

        target_image = '{0}/cine_sax_ED.nii.gz'.format(self.results_dir)
        target_image_crop = '{0}/cine_sax_ED_crop_auto.nii.gz'.format(self.results_dir)
        ...
        target_image_crop = '{0}/cine_sax_ES_crop_auto.nii.gz'.format(self.results_dir)
        os.system('{0}/region {1} {2} -ref {3}'.format(self.irtk_dir, target_image, target_image_crop, fusion_crop))
        
        # Mask for the heart region to save computation and improve accuracy
        mask = '{0}/mask_auto.nii.gz'.format(self.results_dir)
        ...
        for fr in ['ED', 'ES']:
            ...
            # Label fusion
            target_image = '{0}/cine_sax_{1}_crop_auto.nii.gz'.format(self.results_dir, fr)
            fusion = '{0}/fusion_pbcc_nreg_{1}_auto.nii.gz'.format(self.results_dir, fr)
            ...
            # Label fusion
            target_image = '{0}/cine_sax_{1}_crop_auto.nii.gz'.format(self.results_dir, fr)
            fusion = '{0}/fusion_pbcc_nreg_label_{1}_auto.nii.gz'.format(self.results_dir, fr)
            par = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'par/pbcc.cfg')
            os.system('label_fusion {0} {1} {2} {3} {4} -method PBCC -par {5}'.format(target_image, self.n_atlas, image_props, label_props, fusion, par))
            ...
            areg_dof = '{0}/areg_label_to_template_{1}_auto.dof.gz'.format(self.dof_dir, fr)
            ...
            myo = '{0}/myo_pbcc_nreg_{1}_auto.nii.gz'.format(self.results_dir, fr)
            os.system('{0}/padding {1} {1} {2} 2 0 -invert'.format(self.irtk_dir, fusion, myo))        

            myo_mesh = '{0}/myo_pbcc_nreg_{1}_auto.vtk'.format(self.results_dir, fr)
            os.system('{0}/mcubes {1} {2} 1'.format(self.irtk_dir, myo, myo_mesh))

            template_myo_mesh = '{0}/myo_{1}_sm.vtk'.format(self.template_dir, fr)
            snreg_dof = '{0}/snreg_to_template_{1}_auto.dof.gz'.format(self.dof_dir, fr)
            ...
            template_label = '{0}/segmentation_{1}_in_ref3_nreg.nii.gz'.format(self.template_dir, fr)
            label_prop = '{0}/template_fit_snreg_{1}_auto.nii.gz'.format(self.results_dir, fr)
            ...
            mesh_prop = '{0}/template_fit_snreg_{1}_auto.vtk'.format(self.results_dir, fr)
            ...
            if fr == 'ED':
                ...
                model_prop = '{0}/AHA17_fit_snreg_{1}_auto.nii.gz'.format(self.results_dir, fr)
                ...
                mesh_prop = '{0}/AHA17_fit_snreg_{1}_auto.vtk'.format(self.results_dir, fr)
