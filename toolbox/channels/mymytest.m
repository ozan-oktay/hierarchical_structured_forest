...
function mymytest( inputImageName, b_normalize, b_fs_fa, factor )
...
loaddependencies();
inputstr   = load_untouch_nii(inputImageName);
inputImage = double(inputstr.img);

mask            = myMaskGenerator(inputImage); %%% Generate Mask for boundaries
wavLengths      = [4.0,6.0,8.0]*factor;  % center frequency of bandpass filter LOP
...
outputImage   = cast(FA_ENG, 'single');
outputstr     = inputstr;
outputstr.img = outputImage;
outputstr.hdr.dime.datatype = 16;
save_untouch_nii(outputstr,sprintf('/homes/oo2113/tmp/ps_fac%f.nii.gz',factor))
