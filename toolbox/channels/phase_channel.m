...
%It uses local phase properties of the signal to
%compute feature assymetry

function [ outputImage ] = phase_channel( inputImage, b_normalize, b_fs_fa, bw_factor )
...
[~,FA,~]    = phaseImageFunction(inputImage,wavLengths,'LOP',b_fs_fa); %%% Generate Phase Image 
FA (mask)   = 0.0;
if (b_normalize)
  FA  = invertImage(FA);
  FA  = myNormalizeImage (FA);
end
outputImage = cast(FA, inputType);
