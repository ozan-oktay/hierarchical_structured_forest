...
%%%% set opts for training (see edgesTrain_3D.m) %%%%
   ...
   opts.gtWidth= 12;                    % [12]  width of ground truth patches     
      opts.nPos= 1e4;                   % [1e6] number of positive patches per tree
      opts.nNeg= 1e4;                   % [1e6] number of negative patches per tree
     opts.nImgs= 6;                     % [75]  maximum number of images to use for training
    opts.nTrees= 4;                     % [16]  number of trees in forest to train
...
opts.nTreesEval= 4;                     % [10]  number of trees to evaluate per location
 ...
 opts.useParfor= 1;                     % [0]   if true train trees in parallel (memory intensive)
  ...
  opts.modelFnm= 'mriSecond_hier_T';    % ['ctmodel'] model filename
