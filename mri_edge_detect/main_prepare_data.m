...
targetdir_training = '/vol/biomedic/users/oo2113/str_hier_forest_mri/mritrainingdata';
...
parfor patientId = 1 : numel (atlasfoldernames)
  ...
  %volume4d = load_untouch_nii(volume4dname); z_spacing = volume4d.hdr.dime.pixdim(4); n_slices = volume4d.hdr.dime.dim(4);  volume4d.img=[];
  ...
  %system(sprintf('transformation %s %s -dofin "Id" -target %s -nn -matchInputType',eslabelname,eslabelname,volume4dname));
  ...
  %system(sprintf('selectLabelId.py --inputname %s --outputname %s -l %s',eslabelname,eslabelname,sprintf('%d ',selectedLabels)));
  ...
  tmp_esimgname = strcat(targetdir_training_tmp,sprintf('/frame%2.2d.nii.gz',es_frame));
  
  target_imgnameED   = strcat(targetdir_training,'/images',sprintf('/%s_ed%d_3D.nii.gz',patientName,ed_frame));
  %target_imgnameES   = strcat(targetdir_training,'/images',sprintf('/%s_es%d_3D.nii.gz',patientName,es_frame));
  target_labelnameED = strcat(targetdir_training,'/groundtruth',sprintf('/%s_ed%d_3D.nii.gz',patientName,ed_frame));
  %target_labelnameES = strcat(targetdir_training,'/groundtruth',sprintf('/%s_es%d_3D.nii.gz',patientName,es_frame));
  
  system (horzcat(convertbin,' ',tmp_edimgname,' ',target_imgnameED,' ','-',image_type));
  %system (horzcat(convertbin,' ',tmp_esimgname,' ',target_imgnameES,' ','-',image_type));
  system (horzcat(convertbin,' ',edlabelname,' '  ,target_labelnameED,' ','-',label_type));
  %system (horzcat(convertbin,' ',eslabelname,' '  ,target_labelnameES,' ','-',label_type));
  
  %%%% Generate Boundaries 
  target_boundnameED = strcat(targetdir_training,'/boundary',sprintf('/%s_ed%d_3D.nii.gz',patientName,ed_frame));
  %target_boundnameES = strcat(targetdir_training,'/boundary',sprintf('/%s_es%d_3D.nii.gz',patientName,es_frame));
  system(horzcat('sobelEdgeDetector.py',' ','--img',' ',target_labelnameED,' ','--output',' ',target_boundnameED));
  %system(horzcat('sobelEdgeDetector.py',' ','--img',' ',target_labelnameES,' ','--output',' ',target_boundnameES));
  ...
  %system(horzcat('headertool',' ',target_imgnameES,' ',target_imgnameES,' ','-reset'));
  system(horzcat('headertool',' ',target_labelnameED,' ',target_labelnameED,' ','-reset'));
  %system(horzcat('headertool',' ',target_labelnameES,' ',target_labelnameES,' ','-reset'));
